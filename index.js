(() => {
  ...
  function notFindClients() {
    ...
    cell.colSpan = '6';
    cell.textContent = 'Clients not found';
  ...
  function createModalForDeleteClients(clientId) {
    ...
    span.textContent = 'Are you sure? This operation will not be reversible';
  ...
  function createModalForm(client = null) {
    function getContactElement(contactInfo = null) {
      ...
      optionAnother.textContent = 'Other';
      input.placeholder = 'Insert the contact data';
      ...
      exitElement.innerHTML = `<svg class="exit-icon" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    function createInputContainer(input) {
      const inputContainer = document.createElement('div');
      inputContainer.classList.add('add__input-container');

      const label = document.createElement('label');
      label.classList.add('add__input-label');
      ...
      input.addEventListener('focus', () => {
        label.classList.add('add__input-label--focused');
        input.setAttribute('placeholder', '');
      });

      input.addEventListener('blur', () => {
        if (input.value === '') {
          label.classList.remove('add__input-label--focused');
    ...
    surnameElement.placeholder = 'Last name*';
    nameElement.placeholder = 'Name*';
    lastNameElement.placeholder = 'Surname';
    contactsButtonElement.textContent = 'Add contact';
    ...
    if (client) {
      titleElement.textContent = 'Change data';
      clientIdElement.textContent = `ID: ${client.id}`;
      cancelButtonElement.textContent = 'Delete client';
      ...
      titleElement.textContent = 'New client';
    ...
    icon.innerHTML = `<svg viewBox="0 0 160 160" height="16" width="16" xmlns="http://www.w3.org/2000/svg" >
    ...
    formWrap.append(createInputContainer(surnameElement));
    formWrap.append(createInputContainer(nameElement));
    formWrap.append(createInputContainer(lastNameElement));
    ...
    saveButtonElement.addEventListener('click', event => {
      ...
      if (!checkName(surnameElement.value) || !checkName(nameElement.value)) {
        ...
        if (!checkName(nameElement.value)) nameElement.classList.add('add__input-error');
        if (!checkName(surnameElement.value)) surnameElement.classList.add('add__input-error');
        errorMsg.textContent = 'Fill the required information!';
      ...
      contactsListElem.forEach((item) => {
        if (item.value.trim() === '') {
          errors++;
          errorMsg.textContent = 'Fill in or delete the contact informations!';
          btnsGroupElement.prepend(errorMsg);
        }
  ...
  function getClientsElement(client) {
    function getContactsElement(array) {
      ...
      array.map(contact => {
        ...
        button.setAttribute('data-tippy-content', `${contact.type}: ${contact.value}`);
    ...
    const loadingChangeIcon = document.createElement('div');
    const loadingDeleteIcon = document.createElement('div');
    ...
    loadingChangeIcon.innerHTML = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    loadingDeleteIcon.innerHTML = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    changeClientButton.textContent = 'Change';
    changeClientButton.prepend(changeIcon);
    deleteClientButton.textContent = 'Delete';
    ...
    loadingChangeIcon.classList.add('clients__table-loadingicon');
    loadingDeleteIcon.classList.add('clients__table-loadingicon');
    ...
    function handleButtonChange() {
      // replacing the original icon
      changeClientButton.replaceChild(loadingChangeIcon, changeIcon);

      createModalForm(client);

      // replace back the icon after a delay
      setTimeout(() => {
        changeClientButton.replaceChild(changeIcon, loadingChangeIcon);
      }, 300);
    }

    function handleButtonDelete() {
      // replacing the original icon
      deleteClientButton.replaceChild(loadingDeleteIcon, deleteIcon);

      createModalForDeleteClients(client.id);

      // replace back the icon after a delay
      setTimeout(() => {
        deleteClientButton.replaceChild(deleteIcon, loadingDeleteIcon);
      }, 300);
    }

    changeClientButton.addEventListener('click', handleButtonChange);

    deleteClientButton.addEventListener('click', handleButtonDelete);

    cellId.textContent = client.id.slice(0, 6);
